...
expected_ruleset="branch | active | 0 | ~DEFAULT_BRANCH |  | deletion,non_fast_forward,pull_request,required_linear_history | 0 true true squash false false"
...
ruleset_query='[.target, .enforcement, (.bypass_actors|length|tostring), (.conditions.ref_name.include|join(",")), ((.conditions.ref_name.exclude // [])|join(",")), ([.rules[].type]|sort|join(",")), ((.rules[]|select(.type=="pull_request")|.parameters) as $p | [$p.required_approving_review_count, $p.dismiss_stale_reviews_on_push, $p.required_review_thread_resolution, (($p.allowed_merge_methods // [])|join("+")), $p.require_code_owner_review, $p.require_last_push_approval] | map(tostring) | join(" "))] | join(" | ")'

refused() { # $1 = gh's error text; a refused ruleset call names the plan limit on private repositories
  echo "$1" >&2
...
ruleset_id() { # GitHub refuses even the read on a private GitHub Free repository
  local out
  out="$(gh api "repos/$repo/rulesets" --jq ".[] | select(.name==\"$ruleset_name\") | .id" 2>&1)" || refused "$out"
  printf '%s' "$out"
}
