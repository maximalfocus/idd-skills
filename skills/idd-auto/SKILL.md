...
Drive an existing exact `{project}` ↔ `{project}-prd` pair through repeated `/idd-plan` → `/idd-issue` → `/idd` → `/idd-land` cycles. Keep one active issue at a time and preserve every constituent duplicate, acceptance, verification, review, merge, cleanup, and reconciliation gate.
...
4. Define completion as every accepted implementation requirement in the PRD mapped to landed or validated tracker evidence, with no corresponding open issue or PR. Preserve `Landed` versus `Validated`; external evaluation, deployment, publication, or a product decision that cannot be completed by this issue workflow is a blocker, not work to fabricate.
...
Never weaken a constituent gate to keep the loop moving. Stop with one precise blocker when authority becomes ambiguous, either tree contains unrelated work, the next issue needs a material product choice, acceptance remains unproved, CI/review/conflict policy is red, external state is required, or PRD reconciliation fails. Do not auto-apply `--accept-residuals`; only a fresh user instruction can accept named gaps.

Declare completion only after a final live audit proves the completion target, both repositories are clean and synchronized on their default branches, no feature branch from the run remains, and `PROGRESS.md` contains the verified issue/PR/squash evidence. Return the created and landed issue/PR URLs, final implementation and PRD commits, verification state, and either `complete` or the single resumable blocker.
