...
argument-hint: "[project-name|--reconstruct|--reconcile]"
---
...
3. Make the delivery slices the increments that actually shipped, in delivery order, each bound in `PROGRESS.md` to its live issue, pull request, and squash commit; record work predating the issue-driven flow as direct commits and never invent issue numbers for it. Run the repository's own verification gate and record its real result, mark nothing `validated` without live acceptance evidence, and state outstanding verification gaps explicitly. Publish through `scripts/init-prd.sh` as greenfield does, then continue in default mode.
...
2. Edit only `PROGRESS.md`, which states current status rather than an accumulated delivery log. Record each verified issue/PR/squash link in the slice's own row instead of an appended note, apply the tracker's existing status semantics, and preserve distinctions such as partially landed, landed, and validated. Replace superseded narrative and stale intermediate status rather than appending beside it; git and GitHub already hold that history. Once every PRD slice has reached its terminal status, compact the tracker to that checkpoint — one row per slice with its status and verified links — so a later PRD revision plans its new slices from a concise baseline; compaction never drops a verified link. Never copy private PRD content into the public repository or claim acceptance not proven by live evidence.
...
Both bootstrap modes require PRD/tracker ID agreement and private visibility readback unless draft-only. Greenfield additionally requires material product decisions settled and no technical-question drift; reconstruct additionally requires a named source commit, every requirement traceable to implemented behavior, historical slices bound to live evidence or direct commits, a recorded real verification result, and no `validated` status without live acceptance evidence. Existing modes require an exact pair and complete live-state reads. Every mode permits one next issue at most, dependency-respecting order, and no invented lifecycle evidence. Reconcile leaves both repositories clean and synchronized with only `PROGRESS.md` changed, current rather than accumulated, and retaining every previously verified link; failure after merge is reported as `landed, PRD reconciliation incomplete` and never rolls back the merge.
