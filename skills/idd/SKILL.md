...
description: >-
  ...
  reconcile modes, issue creation, landing, autonomous runs, publication, and evolution are routed
  only when the request names that action.
...
1. Resolve this installed skill directory and its siblings `idd-implement`, `idd-plan`, `idd-issue`,
   `idd-land`, `idd-auto`, `idd-acceptance`, `idd-publish`, and `idd-evolve`; a missing sibling the
   chosen route needs stops with its exact skill name.
2. Take the request from the invocation arguments or, on runners that do not inject arguments into
   skills, from the user's message. Read root `AGENTS.md`/`CLAUDE.md`.
3. Note only what routing needs: git repository or not, an issue number or GitHub issue URL in the
   request, an exact `{project}-prd` sibling, and whether the request names a phase in its words.
...
- The selected phase executes only the constituent sequence its own contract authorizes, preserving
  every constituent gate; `/idd` never starts `idd-auto` or `idd-publish` by inference.
