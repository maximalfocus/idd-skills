...
`/idd` stops at an open PR; `/idd-land` is the separate destructive lifecycle step. An explicit invocation authorizes squash merge, issue closure, default-branch refresh, and deletion of that PR's same-repository remote/local feature branch. It does not authorize force-push, bypassing failed checks/reviews/conflicts, deployment, or landing any other issue.
...
Return only the issue/PR URLs, squash commit, closure state, deleted branch names, current default branch, accepted residuals (if any), and any incomplete postcondition.
